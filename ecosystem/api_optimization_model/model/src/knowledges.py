...
knowledges = {
    ...
    "aclgrphBuildModel": "检查aclgrphBuildModel接口是否有传入PRECISION_MODE、EXEC_DISABLE_REUSED_MEMORY、AUTO_TUNE_MODE三个参数，若没有，则无需处理；"
                         "若有，则检查下init中是否已配置，若未配置，建议在build接口的option中进行删除，逻辑跟原先一致，若已配置，需要用户根据需要进行配置修改，若全局仅生效一次，"
    ...
    "aclgrphParseCaffe": "请检查参数是否有配置INPUT_FORMAT、INPUT_SHARP、INPUT_DYNAMIC_INPUT、OP_NAME_MAP、OUTPUT_TYPE、LOG_LEVEL，如果有则需要删除这些参数",
    "aclgrphParseTensorFlow": "请检查参数是否有配置INPUT_FORMAT、INPUT_SHARP、INPUT_DYNAMIC_INPUT、OP_NAME_MAP、OUTPUT_TYPE、LOG_LEVEL，如果有则需要删除这些参数",
    "aclgrphParseONNX": "请检查参数是否有配置INPUT_FORMAT、INPUT_SHARP、INPUT_DYNAMIC_INPUT、OP_NAME_MAP、OUTPUT_TYPE、LOG_LEVEL，如果有则需要删除这些参数",
    "aclgrphParseONNXFromMem": "请检查参数是否有配置INPUT_FORMAT、INPUT_SHARP、INPUT_DYNAMIC_INPUT、OP_NAME_MAP、OUTPUT_TYPE、LOG_LEVEL，如果有则需要删除这些参数",
    ...
    "acl.util.numpy_to_ptr": "如果使用了该接口，若不想修改代码，则需要升级运行环境为python>=3.8及numpy>=1.22.0，若要修改代码，则可用acl.util.bytes_to_ptr替代",
    "acl.util.numpy_contiguous_to_ptr": "如果使用了该接口，若不想修改代码，则需要升级运行环境为python>=3.8及numpy>=1.22.0，若要修改代码，则可用acl.util.bytes_to_ptr替代",
    "acl.util.ptr_to_numpy": "如果使用了该接口，若不想修改代码，则需要升级运行环境为python>=3.8及numpy>=1.22.0，若要修改代码，则可用acl.util.ptr_to_bytes替代",
    "acl.util.set_attr_list_int": "如果使用了该接口，若不想修改代码，则需要升级运行环境为python>=3.8及numpy>=1.22.0，若要修改代码，接口的numpy输入可以替换成list输入",
    "acl.util.set_attr_list_bool": "如果使用了该接口，若不想修改代码，则需要升级运行环境为python>=3.8及numpy>=1.22.0，若要修改代码，接口的numpy输入可以替换成list输入",
    "acl.util.set_attr_list_float": "如果使用了该接口，若不想修改代码，则需要升级运行环境为python>=3.8及numpy>=1.22.0，若要修改代码，接口的numpy输入可以替换成list输入",
    "acl.util.set_attr_list_list_int ": "如果使用了该接口，若不想修改代码，则需要升级运行环境为python>=3.8及numpy>=1.22.0，若要修改代码，接口的numpy输入可以替换成list输入",
}
